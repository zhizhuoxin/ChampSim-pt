...
uint32_t O3_CPU::add_to_ifetch_buffer(ooo_model_instr *arch_instr) {
    ...
    */
    ...
    uint64_t instr_pa = va_to_pa(cpu, IFETCH_BUFFER.entry[index].instr_id, IFETCH_BUFFER.entry[index].ip,
...
int O3_CPU::prefetch_code_line(uint64_t pf_v_addr) {
    ...
    if (L1I.PQ.occupancy < L1I.PQ.SIZE) {
        ...
        uint64_t pf_pa =
                (va_to_pa(cpu, 0, pf_v_addr, pf_v_addr >> LOG2_PAGE_SIZE, 1) & (~((1 << LOG2_PAGE_SIZE) - 1))) |
...
void O3_CPU::do_scheduling(uint32_t rob_index) {
    ...
    if (ROB.entry[rob_index].is_memory)
        ...
        if (ROB.entry[rob_index].reg_ready) {
            ...
            DP (if (warmup_complete[cpu]) {
                cout << "[RTE1] " << __func__ << " instr_id: " << ROB.entry[rob_index].instr_id << " rob_index: "
                     << rob_index << " is added to RTE1";
...
void O3_CPU::add_load_queue(uint32_t rob_index, uint32_t data_index) {
    ...
    if (forwarding_index != SQ.SIZE) { // we have a store-to-load forwarding
        ...
        } else; // store is not executed yet, forwarding will be handled by execute_store()
    }
    ...
    if (LQ.entry[lq_index].virtual_address &&
        (LQ.entry[lq_index].producer_id == UINT64_MAX)) { // not released and no forwarding
...
void O3_CPU::mem_RAW_dependency(uint32_t prior, uint32_t current, uint32_t data_index, uint32_t lq_index) {
    for (uint32_t i = 0; i < MAX_INSTR_DESTINATIONS; i++) {
        ...
        if (ROB.entry[prior].destination_memory[i] ==
            ROB.entry[current].source_memory[data_index]) { //  store-to-load forwarding check
            ...
            for (uint32_t j = 0; j < NUM_INSTR_SOURCES; j++) { // which one is dependent?
                if (ROB.entry[dependent].source_memory[j] && ROB.entry[dependent].source_added[j]) {
                    if (ROB.entry[dependent].source_memory[j] ==
                        SQ.entry[sq_index].virtual_address) { // this is required since a single instruction can issue multiple loads
            ...
            if (ROB.entry[rob_index].registers_index_depend_on_me[j].search(i)) {
                ...
                if (ROB.entry[i].num_reg_dependent == 0) {
                    ...
                    if (ROB.entry[i].is_memory)
                        ...
                        DP (if (warmup_complete[cpu]) {
                            cout << "[RTE0] " << __func__ << " instr_id: " << ROB.entry[i].instr_id << " rob_index: "
                                 << i << " is added to RTE0";
...
void O3_CPU::complete_instr_fetch(PACKET_QUEUE *queue, uint8_t is_it_tlb) {
    ...
    if (is_it_tlb) {
        uint64_t instruction_physical_address =
                (queue->entry[index].instruction_pa << LOG2_PAGE_SIZE) | (complete_ip & ((1 << LOG2_PAGE_SIZE) - 1));
        ...
        for (uint32_t j = 0; j < IFETCH_BUFFER.SIZE; j++) {
            if (((IFETCH_BUFFER.entry[j].ip) >> LOG2_PAGE_SIZE) == ((complete_ip) >> LOG2_PAGE_SIZE)) {
                ...
                uint64_t instr_pa = (queue->entry[index].instruction_pa << LOG2_PAGE_SIZE) |
    ...
    if (is_it_tlb) {
        ...
        ROB.entry[rob_index].instruction_pa = (queue->entry[index].instruction_pa << LOG2_PAGE_SIZE) |
    ...
    DP (if (warmup_complete[cpu]) {
        cout << "[" << queue->NAME << "] " << __func__ << " cpu: " << cpu << " instr_id: "
    ...
    if (queue->entry[index].instr_merged) {
        ITERATE_SET(i, queue->entry[index].rob_index_depend_on_me, ROB_SIZE) {
            ...
            if (is_it_tlb) {
                ...
                ROB.entry[i].instruction_pa = (queue->entry[index].instruction_pa << LOG2_PAGE_SIZE) |
                                              (ROB.entry[i].ip & ((1 << LOG2_PAGE_SIZE) - 1)); // translated address
            } else
            ...
            DP (if (warmup_complete[cpu]) {
                cout << "[" << queue->NAME << "] " << __func__ << " cpu: " << cpu << " instr_id: "
                     << ROB.entry[i].instr_id;
                cout << " ip: " << hex << ROB.entry[i].ip << " address: " << ROB.entry[i].instruction_pa << dec;
                cout << " translated: " << +ROB.entry[i].translated << " fetched: " << +ROB.entry[i].fetched
...
void O3_CPU::complete_data_fetch(PACKET_QUEUE *queue, uint8_t is_it_tlb) {
    ...
    if (is_it_tlb) { // DTLB
        ...
        if (queue->entry[index].type == RFO) {
            SQ.entry[sq_index].physical_address = (queue->entry[index].data_pa << LOG2_PAGE_SIZE) |
            ...
            DP (if (warmup_complete[cpu]) {
                ...
                cout << " DTLB_FETCH_DONE translation: " << +SQ.entry[sq_index].translated << hex << " page: "
                     << (SQ.entry[sq_index].physical_address >> LOG2_PAGE_SIZE);
                cout << " full_addr: " << SQ.entry[sq_index].physical_address << dec << " store_merged: "
                     << +queue->entry[index].store_merged;
                cout << " load_merged: " << +queue->entry[index].load_merged << endl;
            });

            handle_merged_translation(&queue->entry[index]);
        } else {
            LQ.entry[lq_index].physical_address = (queue->entry[index].data_pa << LOG2_PAGE_SIZE) |
            ...
            DP (if (warmup_complete[cpu]) {
                cout << "[RTL1] " << __func__ << " instr_id: " << LQ.entry[lq_index].instr_id << " rob_index: "
                     << LQ.entry[lq_index].rob_index << " is added to RTL1";
                cout << " head: " << RTL1_head << " tail: " << RTL1_tail << endl;
            });

            DP (if (warmup_complete[cpu]) {
                cout << "[ROB] " << __func__ << " load instr_id: " << LQ.entry[lq_index].instr_id;
                cout << " DTLB_FETCH_DONE translation: " << +LQ.entry[lq_index].translated << hex << " page: "
                     << (LQ.entry[lq_index].physical_address >> LOG2_PAGE_SIZE);
                cout << " full_addr: " << LQ.entry[lq_index].physical_address << dec << " store_merged: "
                     << +queue->entry[index].store_merged;
                cout << " load_merged: " << +queue->entry[index].load_merged << endl;
            });
        ...
        if (queue->entry[index].type == RFO)
            ...
            DP (if (warmup_complete[cpu]) {
                cout << "[ROB] " << __func__ << " load instr_id: " << LQ.entry[lq_index].instr_id;
                cout << " L1D_FETCH_DONE fetched: " << +LQ.entry[lq_index].fetched << hex << " address: "
                     << (LQ.entry[lq_index].physical_address >> LOG2_BLOCK_SIZE);
                cout << " full_addr: " << LQ.entry[lq_index].physical_address << dec << " remain_mem_ops: "
                     << ROB.entry[rob_index].num_mem_ops;
                cout << " load_merged: " << +queue->entry[index].load_merged << " inflight_mem: "
...
void O3_CPU::handle_o3_fetch(PACKET *current_packet, uint32_t cache_type) {
    ...
    if (cache_type == 0) { // DTLB
        ...
        if (current_packet->type == RFO) {
            SQ.entry[sq_index].physical_address = (current_packet->data_pa << LOG2_PAGE_SIZE) |
                                                  (SQ.entry[sq_index].virtual_address &
                                                   ((1 << LOG2_PAGE_SIZE) - 1)); // translated address
            SQ.entry[sq_index].translated = COMPLETED;
            ...
            DP (if (warmup_complete[cpu]) {
                ...
                cout << " DTLB_FETCH_DONE translation: " << +SQ.entry[sq_index].translated << hex << " page: "
                     << (SQ.entry[sq_index].physical_address >> LOG2_PAGE_SIZE);
                cout << " full_addr: " << SQ.entry[sq_index].physical_address << dec << " store_merged: "
                     << +current_packet->store_merged;
                cout << " load_merged: " << +current_packet->load_merged << endl;
            });

            handle_merged_translation(current_packet);
        } else {
            LQ.entry[lq_index].physical_address = (current_packet->data_pa << LOG2_PAGE_SIZE) |
                                                  (LQ.entry[lq_index].virtual_address &
                                                   ((1 << LOG2_PAGE_SIZE) - 1)); // translated address
            LQ.entry[lq_index].translated = COMPLETED;
        ...
        if (current_packet->type == RFO)
            ...
            DP (if (warmup_complete[cpu]) {
                ...
                cout << " full_addr: " << LQ.entry[lq_index].physical_address << dec << " remain_mem_ops: "
                     << ROB.entry[rob_index].num_mem_ops;
                cout << " load_merged: " << +current_packet->load_merged << " inflight_mem: " << inflight_mem_executions
                     << endl;
            });
...
void O3_CPU::handle_merged_translation(PACKET *provider) {
    if (provider->store_merged) {
        ITERATE_SET(merged, provider->sq_index_depend_on_me, SQ.SIZE) {
            ...
            SQ.entry[merged].physical_address = (provider->data_pa << LOG2_PAGE_SIZE) |
            ...
            DP (if (warmup_complete[cpu]) {
                ...
                cout << " DTLB_FETCH_DONE translation: " << +SQ.entry[merged].translated << hex << " page: "
                     << (SQ.entry[merged].physical_address >> LOG2_PAGE_SIZE);
                cout << " full_addr: " << SQ.entry[merged].physical_address << dec << " by instr_id: "
                     << +provider->instr_id << endl;
            });
        }
    }
    if (provider->load_merged) {
        ITERATE_SET(merged, provider->lq_index_depend_on_me, LQ.SIZE) {
            ...
            LQ.entry[merged].physical_address = (provider->data_pa << LOG2_PAGE_SIZE) |
            ...
            DP (if (warmup_complete[cpu]) {
                cout << "[RTL1] " << __func__ << " instr_id: " << LQ.entry[merged].instr_id << " rob_index: "
                     << LQ.entry[merged].rob_index << " is added to RTL1";
                cout << " head: " << RTL1_head << " tail: " << RTL1_tail << endl;
            });

            DP (if (warmup_complete[cpu]) {
                cout << "[ROB] " << __func__ << " load instr_id: " << LQ.entry[merged].instr_id;
                cout << " DTLB_FETCH_DONE translation: " << +LQ.entry[merged].translated << hex << " page: "
                     << (LQ.entry[merged].physical_address >> LOG2_PAGE_SIZE);
                cout << " full_addr: " << LQ.entry[merged].physical_address << dec << " by instr_id: "
                     << +provider->instr_id << endl;
            });
        }
    }
...
void O3_CPU::handle_merged_load(PACKET *provider) {
    ITERATE_SET(merged, provider->lq_index_depend_on_me, LQ.SIZE) {
        ...
        DP (if (warmup_complete[cpu]) {
            cout << "[ROB] " << __func__ << " load instr_id: " << LQ.entry[merged].instr_id;
            cout << " L1D_FETCH_DONE translation: " << +LQ.entry[merged].translated << hex << " address: "
    ...
    for (uint32_t n = 0; n < RETIRE_WIDTH; n++) {
        ...
        if (num_store) {
            if ((L1D.WQ.occupancy + num_store) <= L1D.WQ.SIZE) {
                ...
                DP (if (warmup_complete[cpu]) {
                    cout << "[ROB] " << __func__ << " instr_id: " << ROB.entry[ROB.head].instr_id << " L1D WQ is full"
                         << endl;
                });
